...
def generate_bi_subcluster(index, A_h, S):
    bi_subcluster = [index]
    n = len(A_h)
    for j in range(n):
        if j not in S and A_h[index,j] == 2:
    ...
    for i in range(n):
        ...
        if H <= (P_1-P_3):
            ...
            if label[i] == 1:
                ...
                SC.append(generate_bi_subcluster(i, A_h, S))
                for index in SC[t]:
                    label[index] = 1
                t += 1
    return SC, S, A_h, NN
...
def mergeable(index1, index2, N_BP):
    if N_BP[index1,index2] == max(N_BP[index1]):
        return True
    else:
        return False
...
if __name__ == '__main__':
    ...
    for i, data in enumerate(load_data()):
        ...
        for SC in C:
            plt.scatter(data.iloc[SC][0], data.iloc[SC][1], s=5, alpha=0.5)
    plt.tight_layout()
    plt.show()
    plt.close()
